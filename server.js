...
// برای اینکه کد بدون فایل خارجی کار کند
const wordsData = {
  categories: [
    {
      name: 'عمومی',
      words: [
        { text: 'سیب', level: 'easy', description: 'میوه‌ای قرمز یا سبز که نماد سلامتی است' },
        { text: 'آسمان', level: 'easy', description: 'فضای بالای سر ما که آبی رنگ است' },
        { text: 'کتابخانه', level: 'medium', description: 'محلی برای نگهداری و امانت گرفتن کتاب' },
        { text: 'دماوند', level: 'medium', description: 'بلندترین قله ایران' },
        { text: 'الگوریتم', level: 'hard', description: 'مجموعه‌ای از دستورالعمل‌ها برای حل مسئله' },
        { text: 'قسطنطنیه', level: 'hard', description: 'نام قدیمی شهر استانبول' }
      ]
    }
  ]
};
...
(async () => {
  try {
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS games (id TEXT PRIMARY KEY, room_id TEXT, deck JSONB, status TEXT, started_at TIMESTAMP, results JSONB DEFAULT '[]');`);
    await pool.query(`CREATE TABLE IF NOT EXISTS game_states (
      game_id TEXT, user_id BIGINT, 
...
function getWordLimits(word) {
  ...
  return {
    // فرمول: 1.5 برابر طول کلمه برای خطای مجاز
    allowedWrong: Math.max(1, Math.ceil(len * 1.5)),
    // فرمول: 1.3 برابر طول کلمه برای راهنمایی مجاز
    allowedHints: Math.max(0, Math.floor(len * 1.3))
  };
}
...
async function createGameState(gameId, userId, deck) {
  ...
  await pool.query(`
    INSERT INTO game_states (game_id, user_id, allowed_wrong, hints_allowed) 
    VALUES ($1,$2,$3,$4) ON CONFLICT DO NOTHING
  `, [gameId, userId, limits.allowedWrong, limits.allowedHints]);
}
...
async function advanceToNextWord(gameId, userId, currentIdx, deck, roomId) {
  ...
  // 5. Emit Clean State (Real-time fix)
  const newState = {
...
io.on('connection', (socket) => {
  ...
  socket.on('game:guess', async ({game_id, user_id, letter}) => {
    ...
    if(targetWord.includes(input)) {
      // CORRECT GUESS: +10 per occurrence
      correct.push(input);
      const scoreDelta = 10 * (targetWord.split(input).length - 1);
