...
const pool = new Pool({
  connectionString,
  ssl: connectionString ? (sslEnabled ? { rejectUnauthorized: false } : undefined) : undefined,
});
...
app.get('/health', async (req, res) => {
  try {
    ...
    res.json({ ok: true, ts: nowTs(), ssl: !!pool.options.ssl });
  } catch (e) {
...
const socketMeta = new Map(); // socket.id -> { room_ids:Set, user_id }
...
io.on('connection', (socket) => {
  ...
  socket.on('game:timer', async ({ game_id, user_id, timer_ms }) => {
    try {
      if (!game_id || !user_id) return;
      const safeMs = safeBigInt(timer_ms, 0);
...
(async () => {
  try {
    ...
    c.release();
    console.log('Postgres reachable. SSL:', !!pool.options.ssl);
  } catch (e) {
  ...
  server.listen(PORT, () => {
    console.log(`Server listening on ${PORT}, WEB_APP_URL=${WEB_APP_URL}`);
  });
})();
