// server.js
// Competitive two-player game with reveal modes, fullname display, score & progress
...
// ---------- Words pack (keep with normal spaces) ----------
  ...
  try {
    ...
    await pool.query(`
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at = NOW();
    `, [uid, user.username || null, user.first_name || null, user.last_name || null, fullname || null, user.language_code || null, user.photo_url || null]);
    res.json({ ok: true });
  } catch (e) {
  ...
  try {
    ...
    if (players.rows.length >= 2 && rn.status === 'waiting') {
      ...
      for (const p of players.rows) {
        ...
        await pool.query(`INSERT INTO game_states (game_id,user_id,current_index,correct_letters,wrong_letters,hints_used,score,guessed_count,allowed_wrong,timer_ms) VALUES ($1,$2,0,'[]','[]',0,0,0,$3,0) ON CONFLICT DO NOTHING;`, [gameId, p.user_id, allowedWrong]);
      }

      await pool.query(`UPDATE rooms SET status='playing' WHERE id=$1;`, [room_id]);
      // include reveal_mode in event
...
io.on('connection', (socket) => {
  ...
  socket.on('room:setRevealMode', async ({ room_id, user_id, reveal_mode }) => {
    ...
    } catch (e) { console.error('room:setRevealMode error', e); }
  });
  ...
  socket.on('game:resume', async ({ game_id, user_id }) => {
    try {
      ...
      // also send players' fullname & scores
      const players = await pool.query(`SELECT gs.user_id, gs.current_index, gs.score, gs.guessed_count, u.fullname FROM game_states gs LEFT JOIN users u ON u.id = gs.user_id WHERE gs.game_id = $1;`, [game_id]);
  ...
  socket.on('game:guess', async ({ game_id, user_id, letter }) => {
    try {
      ...
      if (positions.length > 0) {
        ...
        const guessedInc = positions.length > 0 ? 1 : 0;
        ...
        // include updated player summary
        const updatedPlayer = await pool.query(`SELECT user_id, score, guessed_count FROM game_states WHERE game_id=$1 AND user_id=$2 LIMIT 1;`, [game_id, user_id]);
        ...
        // send private wrong only
        socket.emit('game:letter:wrong', { user_id, letter: normalized, wrongCount: wrongLetters.length });
      }

      // solved check (ignore spaces)
      ...
      if (solved) {
        ...
        if (nextIndex >= deckLen) {
          ...
          if (roomId) {
            const states = await pool.query(`SELECT gs.user_id, gs.current_index, gs.score, u.fullname FROM game_states gs LEFT JOIN users u ON u.id = gs.user_id WHERE gs.game_id=$1;`, [game_id]);
  ...
  socket.on('game:hint', async ({ game_id, user_id }) => {
    try {
      ...
      await pool.query(`UPDATE game_states SET correct_letters=$3::jsonb, hints_used=hints_used+1, score=GREATEST(score-$4,0), guessed_count=guessed_count+1, last_update=NOW() WHERE game_id=$1 AND user_id=$2;`, [game_id, user_id, JSON.stringify(correctLetters), penalty]);
