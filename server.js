...
IF NOT EXISTS results JSONB DEFAULT '[]';`,
...
async function createGameState(gameId, userId, deck) {
  // محاسبه تعداد مجاز خطا (1.5 برابر طول کلمه)
...
app.post('/stats/leaderboard', async (req, res) => {
  try {
   ...
   FROM users u 
      ...
      GROUP BY u.id, u.fullname, u.photo_url
      HAVING COUNT(DISTINCT gs.game_id) > 0
      ORDER BY total_score DESC, games_played DESC, avg_score DESC
      LIMIT 100
    `);
...
// تاریخچه بازی‌ها با جزئیات کامل و عملکرد بهینه
app.post('/stats/history', async (req, res) => {
  ...
  try {
   ...
   g.deck
    ...
    const historyResult = await pool.query(historyQuery, [user_id]);
...
// پیدا کردن امتیاز کاربر جاری
        const userScore = playersResult.rows.find(p => String(p.user_id) === String(user_id))?.score ||
0;
...
let deck = game.deck;
        ...
        const playersWithStats = playersResult.rows.map(player => {
          let wrong_letters = player.wrong_letters;
          if (typeof wrong_letters === 'string') {
            try {
              wrong_letters = JSON.parse(wrong_letters);
...
[],
          deck: deck ||
[],
          player_stats: playersWithStats
        };
})
    );
    
    res.json({ ok: true, games: gamesWithPlayers });
} catch(e) { 
...
async function getGameStates(gameId) {
  const q = await pool.query(`
    SELECT gs.user_id, gs.current_index, gs.score, gs.guessed_count, gs.hints_used, gs.hints_allowed, 
           gs.wrong_letters, gs.allowed_wrong, u.fullname, u.photo_url
...
return;
  ...
  // محاسبه تعداد مجاز خطا برای کلمه بعدی (1.5 برابر طول کلمه)
...
if (roomId) {
    // FIX: Changed event name to game:word:advanced to trigger client-side board reset
    io.to(roomId).emit('game:word:advanced', { game_id: gameId, by_user: userId, nextIndex, states: playersState });
...
{ room_id, count: players.length, players });
    } catch (error) {
...
socket.on('game:guess', async ({ game_id, user_id, letter }) => {
    try {
     ...
     const idx = Number(gs.current_index) || 0;
      ...
      let wrongLetters = Array.isArray(gs.wrong_letters) ?
gs.wrong_letters : JSON.parse(gs.wrong_letters || '[]');
...
socket.on('game:hint', async ({ game_id, user_id }) => {
  ...
  const idx = Number(gs.current_index) || 0;
      ...
      let correctLetters = Array.isArray(gs.correct_letters) ? gs.correct_letters : JSON.parse(gs.correct_letters || '[]');
      const candidates 
= currentWord.split('').filter(ch => ch.trim() !== '' && ch !== '\u200c' && !correctLetters.includes(ch));
