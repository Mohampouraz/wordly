  ...
  try {
    ...
    const gamesWithPlayers = await Promise.all(
      historyResult.rows.map(async (game) => {
        ...
        const userScore = playersResult.rows.find(p => String(p.user_id) === String(user_id))?.score || 0;
...
// SOCKET LOGIC (با بهبود در انتقال real-time کلمات)
...
async function getGameStates(gameId) {
  const q = await pool.query(`
    SELECT gs.user_id, gs.current_index, gs.score, gs.guessed_count, gs.hints_used, gs.hints_allowed, 
           gs.wrong_letters, gs.allowed_wrong, u.fullname 
...
async function advanceToNextWord(gameId, userId, currentIdx, deck, roomId) {
  ...
  const allowedWrongNext = Math.max(1, ceil(strictLen * 1.5)); // 1.5 برابر طول کلمه
  ...
  // ارسال event جدید برای انتقال به کلمه بعدی
  if (roomId) {
